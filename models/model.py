...
class MaskRCNN(nn.Module):
    ...
    def detect(self, images, mold_image=True, image_metas=None):
        ...
        detections, mrcnn_mask, depth_np = self.predict([molded_images, image_metas], mode='inference')
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers, depth_weight=1, optim_choice="SGD"):
        ...
        for epoch in range(self.epoch+1, epochs+1):
            ...
            # Training
            loss, loss_rpn_class, loss_rpn_bbox, loss_mrcnn_class, loss_mrcnn_bbox, loss_mrcnn_mask, loss_depth = self.train_epoch(train_generator, optimizer, self.config.STEPS_PER_EPOCH, depth_weight)

            # Validation
            val_loss, val_loss_rpn_class, val_loss_rpn_bbox, val_loss_mrcnn_class, val_loss_mrcnn_bbox, val_loss_mrcnn_mask, val_loss_depth = self.valid_epoch(val_generator, self.config.VALIDATION_STEPS, depth_weight)

            # Statistics
            self.loss_history.append([loss, loss_rpn_class, loss_rpn_bbox, loss_mrcnn_class, loss_mrcnn_bbox, loss_mrcnn_mask, loss_depth])
            self.val_loss_history.append([val_loss, val_loss_rpn_class, val_loss_rpn_bbox, val_loss_mrcnn_class, val_loss_mrcnn_bbox, val_loss_mrcnn_mask, val_loss_depth])
